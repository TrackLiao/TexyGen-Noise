...
def parse_cmd(argv):
    try:
        opts, args = getopt.getopt(argv, "hg:t:d:f:")
        ...
        if '-h' in opt_arg.keys():
            ...
            print('       python main.py -g <gan_type> -t real -d <your_data_location> -f <your_fake_data_location>')
            sys.exit(0)
        ...
        if not '-t' in opt_arg.keys():
            ...
            if opt_arg['-t'] == 'real' and '-d' in opt_arg.keys():
                if opt_arg['-t'] == 'real' and '-f' in opt_arg.keys():
                    gan_func(opt_arg['-d'], opt_arg['-f'])
            else:
                gan_func()
...
if __name__ == '__main__':
    ...
    # opts, args = getopt.getopt(sys.argv[1:], "hg:t:d:f:")
    # opt_arg = dict(opts)
    # print(opt_arg['-f'])
